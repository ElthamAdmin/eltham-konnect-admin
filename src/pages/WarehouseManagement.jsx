...
function WarehouseManagement() {
  ...
  const updateStatus = async (trackingNumber, newStatus) => {
    try {
      const res = await axios.put(
        `${API}/api/packages/${trackingNumber}/status`,
        { status: newStatus }
      );
  ...
  const filteredPackages = useMemo(() => {
    return packages.filter((pkg) =>
      `${pkg.trackingNumber} ${pkg.customerEkonId} ${pkg.customerName} ${pkg.courier} ${pkg.status} ${pkg.warehouseLocation || ""}`
        .toLowerCase()
  ...
  return (
    <div>
      ...
      <input
        type="text"
        placeholder="Search by tracking number, EKON ID, customer, courier, status, or location"
        value={searchTerm}
